...
$categoryName = "SFA.Apprenticeships.WorkerRoles.VacancyEtl"
$categoryHelp = "Performance counters related to Vacancy Etl Worker Role"
$categoryType = [System.Diagnostics.PerformanceCounterCategoryType]::SingleInstance

Function CreatePerformanceCounterCategory($vacancyEtlCount){
    $objCCDC = New-Object System.Diagnostics.CounterCreationDataCollection
  
    $objCCD1 = CreatePerformanceCounter "VacancyEtlExecutions" "Number of VacancyEtl executions"
    ...
    CreatePerformanceCounterInstance "VacancyEtlExecutions" $vacancyEtlCount
}
...
Else
{
    ...
    $vacancyEtlRunsCount = GetCounterValue "VacancyEtlExecutions"
    
    [Diagnostics.PerformanceCounterCategory]::Delete($categoryName)    
    CreatePerformanceCounterCategory $vacancyEtlRunsCount
}