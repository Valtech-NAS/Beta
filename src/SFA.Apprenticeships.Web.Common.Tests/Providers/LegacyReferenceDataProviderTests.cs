...
namespace SFA.Apprenticeships.Web.Common.Tests.Providers
{
    ...
    public class LegacyReferenceDataProviderTests
    {
        ...
        public void FixtureSetup()
        {
            ...
            _service.Setup(x => x.GetReferenceData(LegacyReferenceDataType.Framework)).Returns(new List<Model.ILegacyReferenceData>{new Model.Framework{ Id = "Framework", Description = "Framework.1" }});
            _service.Setup(x => x.GetReferenceData(LegacyReferenceDataType.County)).Returns(new List<Model.ILegacyReferenceData> { new Model.Framework { Id = "County", Description = "County.1" } });
            _service.Setup(x => x.GetReferenceData(LegacyReferenceDataType.ErrorCode)).Returns(new List<Model.ILegacyReferenceData> { new Model.Framework { Id = "ErrorCode", Description = "ErrorCode.1" } });
            _service.Setup(x => x.GetReferenceData(LegacyReferenceDataType.LocalAuthority)).Returns(new List<Model.ILegacyReferenceData> { new Model.Framework { Id = "LocalAuthority", Description = "LocalAuthority.1" } });
            _service.Setup(x => x.GetReferenceData(LegacyReferenceDataType.Region)).Returns(new List<Model.ILegacyReferenceData> { new Model.Framework { Id = "Region", Description = "Region.1" } });           
        }
